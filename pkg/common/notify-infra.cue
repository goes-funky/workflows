...
#job_infra_dispatch: #job & {
    ...
    if: "(github.event_name == 'push' || (github.repository == 'goes-funky/integrations' && github.event_name == 'workflow_dispatch')) && github.ref == 'refs/heads/main' && vars.AWS_INFRA_DISPATCH_ROLE != ''"
    steps: [
        {
        ...
        },
        {
            ...
            run: """
                ...
                target=infra
                if [[ "$GITHUB_REPOSITORY" == 'goes-funky/integrations' ]]; then
                  target=integrations
                  service="${repository#"$EXPECTED_REGISTRY/cdata/"}"
                  if [[ ! "$service" =~ ^[a-z0-9]+(-[a-z0-9]+)*$ || "$repository" != "$EXPECTED_REGISTRY/cdata/$service" || ! "$tag" =~ ^[a-fA-F0-9]{7,40}-[0-9]+(\\.[0-9]+)*-[0-9]{12}([0-9]{2})?$ ]]; then
                    echo 'Expected a published CData connector release' >&2
                    exit 1
                  fi
                  # CData tags identify the wrapper revision, not the integrations commit.
                else
                  if [[ "$repository" != "$EXPECTED_REGISTRY/$service" || ! "$tag" =~ ^[a-fA-F0-9]{7,40}-[0-9]+$ ]]; then
                    echo 'Expected a published service release' >&2
                    exit 1
                  fi
                  short_sha="${tag%%-*}"
                ...
                if [[ ! "$digest" =~ ^sha256:[a-f0-9]{64}$ ]]; then
                  echo 'Expected a published image digest' >&2
                  exit 1
                fi
                jq -n --arg target "$target" --arg service "$service" --arg tag "$tag" --arg digest "$digest" \\
                  --arg source_run "$GITHUB_SERVER_URL/$GITHUB_REPOSITORY/actions/runs/$GITHUB_RUN_ID" \\
                  '{ref: "main", inputs: {target: $target, service: $service, environment: "both", tag: $tag, digest: $digest, automatic: "true", "source-run": $source_run, "dry-run": "false"}}' \\
