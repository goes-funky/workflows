    ...
    steps: [
        {
            ...
            with: {
                ...
                "role-to-assume": "arn:aws:iam::119462788859:role/github-infra-dispatch"
        ...
        },
        {
            ...
            run: """
                ...
                if [[ "$repository" != "$EXPECTED_REGISTRY/$service" || ! "$tag" =~ ^[a-fA-F0-9]{7,40}-[0-9]+(-[A-Za-z0-9-]+)?$ || ! "$digest" =~ ^sha256:[a-f0-9]{64}$ ]]; then
                  echo 'Expected a published release image with a digest' >&2
                  exit 1
                fi
                short_sha="${tag%%-*}"
                if [[ "$short_sha" != "7ee2e81" ]]; then
                ...
                jq -n --arg service "$service" --arg tag "$tag" --arg digest "$digest" \\
                  ...
                  '{ref: "aliaksei/test-infra-dispatch", inputs: {service: $service, environment: "dev", tag: $tag, digest: $digest, automatic: "false", "source-run": $source_run, "dry-run": "true"}}' \\
                ...
                for attempt in 1 2 3; do
                  if gh api --method POST repos/goes-funky/infra/actions/workflows/aws-image-update-branches.yaml/dispatches --input "$RUNNER_TEMP/infra-dispatch.json"; then
                    echo 'Requested DEV dry-run in infra'
                    exit 0
                  fi
                  sleep "$attempt"
                done
