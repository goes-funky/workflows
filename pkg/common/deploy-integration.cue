            ...
            steps: [
                ...
                #with.skaffold_cache.step,
                {
                    ...
                    with: {
                        ...
                        path: "./code/${{ inputs.repo-path }}/dist"
                    }
                },
                #with.ssh_agent.step,
                #with.gcloud.step & {
                ...
                #with.kube_tools.step,
                {
                    ...
                    run: """
                        CONTAINER_NAME=$(cd ./code/${{ inputs.repo-path }} && basename -s .git "$(git remote get-url origin)") && echo "CONTAINER_NAME=$CONTAINER_NAME" >> "$GITHUB_ENV"
                        SHORT_SHA="$(git -C ./code/${{ inputs.repo-path }} rev-parse --short HEAD)" && echo "SHORT_SHA=$SHORT_SHA" >> "$GITHUB_ENV"
                        COMMIT_SHA="$(git -C ./code/${{ inputs.repo-path }} rev-parse HEAD)" && echo "COMMIT_SHA=$COMMIT_SHA" >> "$GITHUB_ENV"
